...
getting `jsb` up and running. Though `jsb` is really simple
...
`jsb` uses [Grunt](http://gruntjs.org) as its build system, so
...
```sh
$ git clone https://github.com/krhae/jsb.git
$ cd jsb
```
...
There are many Bower packages used by `jsb`, like Twitter Bootstrap
...
In the future, should you want to add a new Bower package to the app, run the
`install` command:
...
Technically, `jsb` is now ready to go.
...
`jsb`. What follows in this section is a quick introduction to the
...
`jsb` also includes [Live Reload](http://livereload.com/), so you no
...
default with `jsb`) you need to tell Live Reload to allow it. Go to
